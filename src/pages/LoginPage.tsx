...
export const LoginPage = () => {
  ...
  return (
    <div className="flex h-screen flex-col justify-center items-center backdrop-blur-sm bg-cover" style={{backgroundImage: `url(${umnBg})`}}>
      ...
      <h1 className="text-center text-red-600">
        Halo semua, Aul suka yg pas ditangan
          ...
          { move == 0 ? <AvatarSelect onClick={(img) => {
            setMove(1);
...
const LoginPage2 = ({avatarImg, onBack}: {avatarImg: string, onBack: () => void}) => {
  const {user, changeData} = useUser();
  const name = useRef<HTMLInputElement>(null);
  const jurusan = useRef<HTMLSelectElement>(null);
  const navigate = useNavigate();
  const handleSubmit = () => {
    if(!(name.current?.value && jurusan.current?.value && jurusan.current.value !== "Jurusan")) {
  ...
  return (
    <div className="flex flex-col md:flex-row justify-center items-center my-14 bg-white p-12 rounded-lg gap-5">
      ...
      <div className="flex flex-col gap-5">
        <input type="name" name="nama" ref={name} className="mt-5 px-3 py-2 
